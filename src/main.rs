...
fn generate_map(data_set: &mut Vec<Flight>) {
    let num_aircrafts = rand::thread_rng().gen_range(MIN_NUM_AIRCRAFTS..=MAX_NUM_AIRCRAFTS);
    for _ in 0..num_aircrafts {
        add_new_flight(data_set);
    }
}

fn move_aircrafts(data_set: &mut Vec<Flight>) {
...
fn read_http_request(stream: &mut TcpStream) -> Vec<String> {
    ...
    let https_request = buf_reader
        .lines()
        .map(|result| result.unwrap())
        .take_while(|lines| !lines.is_empty())
        .collect();

    println!("Request: {:#?}", https_request);
